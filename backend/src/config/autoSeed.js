...
async function runAutoSeed(prisma) {
  try {
    ...
    if (existingUsers > 0) {
      console.log('🌾 Database đã có dữ liệu, giữ nguyên dữ liệu hiện tại.');
      return { seeded: false, message: 'Database đã có dữ liệu.' };
    }

    console.log('🌾 Database trống, đang tự động nạp dữ liệu mẫu Làng Giao Tác (TDP 9 Thuận Lộc)...');
    ...
    // 2. Tạo Mốc Lịch sử (HistoryTimeline)
    const historyData = [
      {
        yearLabel: 'Năm 1685',
    ...
    // 4. Tạo Bài viết cộng đồng (Posts)
    ...
    console.log('🎉 Tự động nạp dữ liệu mẫu ban đầu thành công! Tài khoản Admin: admin@langgiaotac.vn / 123456');
    return { seeded: true, message: 'Đã khởi tạo dữ liệu mẫu và tài khoản Admin thành công.' };
  } catch (err) {
